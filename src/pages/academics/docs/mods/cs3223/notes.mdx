...
{/* TODO: same, why dedupe only T not 2T rw? */}
...
**NOTE**: output write cost ignored since worst case cross everything outweigh any approach
...
- if join on attribute A and index on Inner.A exists (all attribs of join), then for each record in Inner search index for corresponding A and join.
- cost = `numPagesOuter` + `numRecordsInner * (numInnerInternalDepth + avgLeafNodesPerKey)`
...
- optimization: if number of buffer pages > number of sorted runs in Inner + outer, no need to continue merge in initial sort, can join already (take smallest page of each sorted run)
