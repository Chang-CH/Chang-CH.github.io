...
Note that for most syntax a,b refer to line numbers, except uses.

1. Follows(a,b): in the same nesting level + in same statement list + statement b appears right after a
...
2. [Structural] Facade pattern: like an API class in front of subsystem, prevent coupling with individual subsystem implementation
3. [behavioral] Visitor: e.g. abstract Person, should only handle person logic (SRP), if also handle export can introduce bugs. 
    have visitor class handle export for all Person concrete classes (`Student` etc.).
