    ...
    def sendFile(self):
        ...
        while low < end:
            ...
            for i in range(WINDOW):
                ...
                if not packet.ack:
                    if packet.seq % 1024 == 0:
                        print("{}MB sent".format(packet.seq // 1024))
            ...
            if end == SEQ_LIMIT:
                print("Epoch ended: approx {}MB sent".format(progress // 1024))
            else:
                print("Epoch ended: {}\% complete".format(progress * 100 // end))

            if low == end:
                break

        print("All files ACK'd, sending close packet")
